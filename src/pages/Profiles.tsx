      ...
      <div className="space-y-3">
        {profiles.map((profile) => {
          ...
          return (
            <div
              key={profile.id}
              className={`rounded-lg border p-4 ${isActive ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white'}`}
            >
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="font-semibold text-gray-900">
                    {profile.id} {isActive && <span className="text-blue-600">(active)</span>}
                  </div>
                  <div className="mt-1 text-sm text-gray-600">
                    Created {formatDate(profile.createdAt)} · Saved {formatDate(profile.updatedAt)}
                  </div>
                  <div className="mt-1 text-sm text-gray-600">
                    💰 {profile.state.coins} coins · {profile.state.history.length} history entries
                  </div>
                </div>
                <button
                  onClick={() => {
